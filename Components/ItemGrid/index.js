...
const SUBTITLE = {
  ...
  epics:
    'Sub-components that build into a Legendary/Mythic.',
...
const ItemGrid = ({ className }) => {
  ...
  const itemsData =
    state.tab === 'all'
      ...
      : Object.entries(
          buildDisplayItems(state.itemsData.all, state.tab)
        );
    ...
    <div className={`${styles.gridFrame} ${className}`}>
      {itemGroups.map(([groupName, items]) => {
        const groupItems = state.stat
          ? items.filter((item) =>
              item.categories.includes(state.stat)
            )
          : items;
